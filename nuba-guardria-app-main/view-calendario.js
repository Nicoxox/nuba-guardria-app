  ...
  const header = el("div", { class: "calendar-header" }, [
    ...
    el("div", { class: "calendar-nav" }, [
      ...
      el(
        ...
        {
          onclick: () => {
            const sel = state.selectedDate || state.today;
            const d = new Date(sel);
            d.setDate(d.getDate() - 1);
            state.selectedDate = d;
            state.calendarMonthOffset = (d.getFullYear() - state.today.getFullYear()) * 12 + (d.getMonth() - state.today.getMonth());
      ...
      el(
        "button",
        {
          onclick: () => {
            ...
            const monthDate = addMonths(state.today, state.calendarMonthOffset);
            const day = (state.selectedDate || state.today).getDate();
            const daysInMonth = new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0).getDate();
            state.selectedDate = new Date(monthDate.getFullYear(), monthDate.getMonth(), Math.min(day, daysInMonth));
            window.dispatchEvent(new Event("nuba-rerender"));
          }
        },
        "‹"
